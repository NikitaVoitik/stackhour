...
app="$base/dioxus/target/release/stackhour-bench-dioxus-desktop"
...
xvfb-run -a -s '-screen 0 1280x800x24 -dpi 96' dbus-run-session -- bash -c \
  '"$1" & pid=$!; sleep 8; import -window root "$2"; kill "$pid"; wait "$pid" || true' \
  _ "$app" "$base/screenshots/dioxus-desktop.png"
identify "$base/screenshots/dioxus-desktop.png"
