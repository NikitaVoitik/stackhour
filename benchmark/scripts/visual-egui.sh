...
capture="$(mktemp -d /tmp/stackhour-egui-capture.XXXXXX)"
...
xvfb-run -a -s '-screen 0 1280x800x24 -dpi 96' bash -c '
  ...
  XDG_RUNTIME_DIR="$runtime" weston --backend=x11-backend.so --width=1280 --height=800 \
    --use-pixman --shell=kiosk-shell.so --socket=wayland-egui --debug --no-config \
    --log="$capture/weston.log" &
  weston_pid=$!
  trap "kill $weston_pid 2>/dev/null || true" EXIT
  for _ in $(seq 1 100); do [[ -S "$runtime/wayland-egui" ]] && break; sleep 0.05; done
  XDG_RUNTIME_DIR="$runtime" WAYLAND_DISPLAY=wayland-egui \
  ...
  sleep 8
  cd "$capture"
  XDG_RUNTIME_DIR="$runtime" WAYLAND_DISPLAY=wayland-egui weston-screenshooter
  shot="$(find "$capture" -maxdepth 1 -name "wayland-screenshot-*.png" -print -quit)"
  [[ -n "$shot" ]]
  mv "$shot" "$base/screenshots/egui.png"
