...
const executable = join(base, "build", "bin", "stackhour-bench-wails");
await mkdir(join(base, "results", "raw"), { recursive: true });
await stat(executable);
async function run(phase, index, prime = false) {
  const xdg = join("/tmp", `stackhour-bench-wails-${phase}`);
  if (phase === "cold") await rm(xdg, { recursive: true, force: true });
  await mkdir(xdg, { recursive: true });
  const log = join(base, "results", "raw", `wails-${phase}-${index}.jsonl`);
  await new Promise((resolve, reject) => {
    const child = spawn(executable, [], {
      ...
      env: {
        ...
        BENCH_LOG: prime ? "/tmp/stackhour-wails-prime.jsonl" : log,
