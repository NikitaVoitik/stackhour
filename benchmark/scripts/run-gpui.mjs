...
async function run(phase, index, prime = false) {
  const xdg = join("/tmp", `stackhour-bench-gpui-${phase}`); if (phase === "cold") await rm(xdg, { recursive: true, force: true }); await mkdir(xdg, { recursive: true });
  const log = join(base, "results", "raw", `gpui-${phase}-${index}.jsonl`);
  await new Promise((resolve, reject) => {
    const child = spawn(compositor, [executable], { stdio: "inherit", env: { ...process.env, BENCH_AUTORUN: "1", BENCH_PHASE: phase, BENCH_RUN_ID: `${phase}-${index}`, BENCH_LOG: prime ? "/tmp/stackhour-gpui-prime.jsonl" : log, BENCH_FIXTURE: join(base, ".fixture"), BENCH_LSP: join(base, "node_modules", ".bin", "typescript-language-server"), XDG_CONFIG_HOME: join(xdg, "config"), XDG_CACHE_HOME: join(xdg, "cache") } });
