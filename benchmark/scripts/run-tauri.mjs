...
async function run(phase, index, prime = false) {
  const xdg = join("/tmp", `stackhour-bench-vue-tauri-${phase}`); if (phase === "cold") await rm(xdg, { recursive: true, force: true }); await mkdir(xdg, { recursive: true });
  const log = join(base, "results", "raw", `vue-tauri-${phase}-${index}.jsonl`);
