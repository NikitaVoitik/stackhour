...
const executable = join(base, "src-tauri", "target", "release", "stackhour-bench-vanilla-tauri");
await mkdir(join(base, "results", "raw"), { recursive: true }); await stat(executable);
async function run(phase, index, prime = false) {
  const xdg = join("/tmp", `stackhour-bench-vanilla-tauri-${phase}`); if (phase === "cold") await rm(xdg, { recursive: true, force: true }); await mkdir(xdg, { recursive: true });
  const log = join(base, "results", "raw", `vanilla-tauri-${phase}-${index}.jsonl`);
