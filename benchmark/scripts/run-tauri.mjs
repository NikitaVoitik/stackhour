...
const executable = join(base, "src-tauri", "target", "release", "stackhour-bench-solid-tauri");
await mkdir(join(base, "results", "raw"), { recursive: true }); await stat(executable);
async function run(phase, index, prime = false) {
  const xdg = join("/tmp", `stackhour-bench-solid-tauri-${phase}`); if (phase === "cold") await rm(xdg, { recursive: true, force: true }); await mkdir(xdg, { recursive: true });
  const log = join(base, "results", "raw", `solid-tauri-${phase}-${index}.jsonl`);
