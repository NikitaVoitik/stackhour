...
const executable = join(base, "egui", "target", "release", "stackhour-bench-egui");
...
async function run(phase, index, prime = false) {
  const xdg = join("/tmp", `stackhour-bench-egui-${phase}`);
  if (phase === "cold") await rm(xdg, { recursive: true, force: true });
  await mkdir(xdg, { recursive: true });
  const log = join(base, "results", "raw", `egui-${phase}-${index}.jsonl`);
  await new Promise((resolve, reject) => {
    const child = spawn(compositor, [executable], {
      stdio: "inherit",
      env: {
        ...process.env,
        BENCH_AUTORUN: "1",
        ...
        BENCH_LOG: prime ? "/tmp/stackhour-egui-prime.jsonl" : log,
