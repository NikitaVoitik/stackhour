...
export default function App() {
  ...
  const [selectedFile, setSelectedFile] = createSignal("src/selected.ts");
  const [treeTop, setTreeTop] = createSignal(0);
  const [editorTop, setEditorTop] = createSignal(0);
  const [symbols, setSymbols] = createSignal<Array<{ name: string; kind: number }>>([]);
  const [scannerText, setScannerText] = createSignal("waiting…");
  const [languageText, setLanguageText] = createSignal("language server initializing…");
    ...
    <div class="shell">
      ...
      <aside class="sidebar">
        ...
        <div class="tree"><div class="tree-content">
          <For each={treeRows()}>{(item) => (
            <div
              class="tree-row"
              classList={{ active: item.path === selectedFile() }}
              data-path={item.path}
      ...
      <main class="main">
        ...
        <section class="editor"><div class="editor-content">
          <For each={editorRows()}>{(item) => (
            <div class="code-row" style={{ transform: `translateY(${(item.row - editorTop()) * contract.lineHeight}px)` }}>
              <span class="line-no">{item.row + 1}</span><span class="code" innerHTML={highlight(item.source)} />
            </div>
          )}</For>
        </div></section>
        <aside class="outline">
          ...
          <div class="outline-list">
            <Show when={symbols().length} fallback={<div class="outline-item">Initializing TypeScript…</div>}>
              <For each={symbols().slice(0, 24)}>{(symbol) => <div class="outline-item"><span class="symbol">◇</span>{symbol.name}</div>}</For>
            </Show>
          </div>
        </aside>
